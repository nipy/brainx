...
from .matlab import run_matlab_script, run_matlab


class SpmInfo(object):
    @setattr_on_read
    def spm_path(self):
        spmdir = run_matlab("disp(['SPMDIR ' spm('dir')])")
        for line in spmdir.splitlines():
            if line.startswith('SPMDIR'):
                return line.split(' ',2)[1]
        raise OSError("SPM path not found")
...
def fltcols(vals):
    ...
    return np.atleast_2d(np.asarray(vals, dtype=float))
